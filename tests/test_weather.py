...
def test_compute_ground_speed(sample_weather):
    gs, heading, wind_u, wind_v = sample_weather.compute_ground_speed(
        ground_distance_m=10.0 * NAUTICAL_MILES_TO_METERS,
        altitude_m=30000 * FEET_TO_METERS,
        tas_ms=200.0,
    )

    assert gs == pytest.approx(192.24156612925196, rel=1e-6)
    assert heading == pytest.approx(234.9032159733623, rel=1e-6)
    assert wind_u == pytest.approx(15.785100100781209, rel=1e-6)
    assert wind_v == pytest.approx(-1.0294373674107957, rel=1e-6)
